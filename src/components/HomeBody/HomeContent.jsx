...
const ChampionsLeauge = () => {
  ...
  // استدعاء الماتشات الخاصه بالابطال
  useEffect(() => {
    const fetchMatches = async () => {
      try {
...
const MatchesDate = () => {
  ...
  useEffect(() => {
    ...
    today.setHours(0, 0, 0, 0); // ضبط التوقيت إلى بداية اليوم

    const filtered = matches.filter((match) => {
      const matchDate = new Date(match.utcDate);
      matchDate.setHours(0, 0, 0, 0); // ضبط التوقيت إلى بداية اليوم

      const dayDiff = (matchDate - today) / (1000 * 60 * 60 * 24); // حساب الفرق بالأيام
  ...
  return (
    <>
      ...
      {/* أزرار الاختيار */}
      <div className="flex space-x-4 mt-4 flex-wrap md:mt-5">
...
const OurChampions = () => {
  ...
  return (
    <>
      ...
      <div className="flex space-x-4 mt-4 flex-wrap md:mt-5 gap-2">
        {Leaugs.map((leauge) => (
          <Link
